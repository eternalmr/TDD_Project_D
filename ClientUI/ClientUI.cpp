...
BEGIN_MESSAGE_MAP(CClientUIApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CClientUIApp::OnAppAbout)
	// 基于文件的标准文档命令
...
CClientUIApp::CClientUIApp() noexcept
{
	// 支持重新启动管理器
	...
	SetAppID(_T("ClientUI.AppID.NoVersion"));
...
BOOL CClientUIApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CClientUIDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架窗口
		RUNTIME_CLASS(CClientUIView));
	if (!pDocTemplate)
		return FALSE;
	...
	m_pMainWnd->MoveWindow(0, 0, Width, Height);
